    ...
    async function fetchData() {
      try {
        ...
        const [statsRes, rankingRes] = await Promise.all([
          axios.get(`${apiUrl}/enem/2023/stats`),
          axios.get(`${apiUrl}/enem/2023/states?limit=10`),
        ]);
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-900/50 backdrop-blur-xl p-6 rounded-2xl border border-slate-800 shadow-2xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">
                PrismaEnem
              </h1>
              <p className="text-slate-400 mt-1 text-sm font-medium">Dashboard Analítico do Gestor</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/40 p-6 rounded-2xl border border-slate-800/60 shadow-xl hover:bg-slate-800/40 transition-colors relative overflow-hidden group">
            ...
            <div className="flex items-center gap-2 mt-4 text-sm relative z-10">
              ...
              {stats && escolaMock.media_geral > stats.media_geral ? (
                <span className="text-emerald-400 text-xs font-bold">+{(escolaMock.media_geral - stats.media_geral).toFixed(1)}</span>
              ) : (
                <span className="text-rose-400 text-xs font-bold">-{(stats!.media_geral - escolaMock.media_geral).toFixed(1)}</span>
              )}
            </div>
          </div>
